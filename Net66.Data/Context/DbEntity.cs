    ...
    public class DbEntity
    {
        ...
        public void DelayedSendingMsg()
        {
            if (Utils.OPENMSG == false)
                return;
            List<PushAlarmMsg> list = null;
            using (DbSysSEC dbEntity = new DbSysSEC("DB_SEC"))
            {
                //var uplist= new List<PushAlarmMsg>();
                list = dbEntity.PushAlarmMsgs.Where(w => w.IsSend == 0).ToList() ?? new List<PushAlarmMsg>();
                if (!list.Any())
                    return;
        ...
        private void SendAlarmMsg(List<PushAlarmMsg> list)
        {
            foreach (var mode in list)
            {
                #region Vxin
